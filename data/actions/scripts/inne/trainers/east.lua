function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    local checkPlayer = {x = doorPos.x + 1, y = doorPos.y, z = doorPos.z}
	local topPlayer = Tile(checkPlayer):getTopCreature()
    
    -- Check where the player is located at.
	if player:getPosition() ~= Position(doorPos.x + 1, doorPos.y, doorPos.z) then
        ...
        if topPlayer then
            ...
            -- Teleport player.
			player:teleportTo(Position(doorPos.x + 1,doorPos.y,doorPos.z))
