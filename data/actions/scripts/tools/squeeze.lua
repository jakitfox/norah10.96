...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if(value ~= -1 and os.clock()-value < 60)then
		doCreatureSay(cid, "The tool jammed. Please wait " .. 60-math.floor(os.clock()-value) .. " seconds before using it again.",TALKTYPE_ORANGE_1)
		return TRUE
	end
	if(math.random(1, 10) == 1)then
		setPlayerStorageValue(cid, 10, os.clock())
		doCreatureSay(cid, "The tool jammed. Please wait 60 seconds before using it again.",TALKTYPE_ORANGE_1)
		return TRUE
	end
	-- Shovel
