local config = {
        ...
        waves = 10,
        questArea = {
                fromPosition = {x = 768, y = 901, z = 7},
                toPosition = {x = 781, y = 913, z = 7}
        },
        summonPositions = {
                {x = 771, y = 904, z = 7},
                {x = 776, y = 903, z = 7},
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
        ...
        for i = 1, #config.demonOakIds do
                totalProgress = totalProgress + math.max(0, player:getStorageValue(config.storages[config.demonOakIds[i]]))
        end
 
        local spectators, hasMonsters = Game.getSpectators(Position(774,906,7), false, false, 9, 9, 6, 6), false
        ...
        player:say(isLastCut and 'HOW IS THAT POSSIBLE?!? MY MASTER WILL CRUSH YOU!! AHRRGGG!' or config.sounds[math.random(#config.sounds)], TALKTYPE_MONSTER_YELL, false, player, Position(774,906,7))
